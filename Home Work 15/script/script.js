...
const filterBalanceUsers = users.filter(function(name) {
    const balanceNumber = parseFloat(name.balance.split('$').join('').split(',').join(''));
    return balanceNumber >= 2000;
});
const usersPhoneNumber = filterBalanceUsers.map(function (name) {
    return name.phone;
});
const sumUsersBalance = users.reduce(function (sum, name) {
    const balanceNumber = parseFloat(name.balance.split('$').join('').split(',').join(''));
...
console.log(`Общая сумма балансов пользователей: ${Math.floor(sumUsersBalance)}$`);